...
# defines the variables of Polymer, or any Company
polymer: public(address)
...
# defining polymer as the company to sell tokens, the length of the auction and when it starts, 
# the token supply, and the starting/ending price of the token 
@external
def __init__(_polymer: address, _total_tokens: uint256, _starting_price: uint256, _auction_start: uint256, _auction_time: uint256, _reserve_price: uint256):

    self.polymer = _polymer
    ...
    # polymer starts with all tokens, is buyer[0] in the hashmap
    self.buyerHash[nbi] = Buyer({holder:_polymer, tokens:_total_tokens, pricePaid:_starting_price})
    self.nextBuyerIndex = nbi + 1
    assert block.timestamp >= _auction_start



# determine how many tokens polymer has
@view
@internal
...
# allows purchase of tokens; polymer gets money, buyer gets tokens
@external
@payable
def buyToken():
    ...
    if self._tokensAvailable() > _tokensBought:
        ...
        # remove tokens from Polymer storage, give them to buyer, log the price paid
        ...
        # polymer storage is now 0, give all tokens to buyer
...
def refund():
    if self.buyerHash[0].tokens == 0 or self.current_price() <= self.reservePrice or (block.timestamp - self.auctionStart) > self.auctionTime:
        ...
        for i in range(ind, ind + 30):
            ...
            # i must account for polymer, the 0th account that doesn't get a refund, so 1 is added to each i to make the loop start at 1
            index:int128 = i + 1
